...
\begin{abstract}
In a CARA economy where each investor compares his wealth to a weighted average
over a network of peers, equilibrium demand scales solve a Katz--Bonacich
system. We characterise how the network aggregates risk aversion through two
limits that hold for every network. At zero comparison intensity the marginal
effect on the effective coefficient is a quadratic form in risk tolerances; when
attention is balanced, in the sense that the influence matrix is doubly
stochastic, that form is a Dirichlet form and the effect is non-negative,
vanishing only if tolerance is constant across links. Under intense comparison
the effective coefficient converges to a stationary average of individual
coefficients, weighted by the stationary distribution of the influence matrix
rather than by population. The two limits organise the rest. Comparison lowers
the effective coefficient whenever stationary influence favours the less risk
averse, which explains sign reversal; it interpolates from the harmonic to the
arithmetic mean when influence is spread evenly, which is the mean-field case;
and it does nothing at all when the network splits into blocks that are
homogeneous in type, which is perfect homophily. A separate result is that the
comparison is exactly neutral, for every network and every profile of
intensities, when net risk aversion is common.
\end{abstract}
...
Second, market clearing here delivers a mean--covariance relation conditional on
the covariance matrix, not a complete system of asset prices; the object we
characterise is the effective market coefficient $\bth$, and we claim nothing
about price levels.
...
$\vec c=M^{-1}\mathbf 1>0$ because no row of $M^{-1}$ can vanish. Market
clearing against a supply with value weights $\vec w\geq0$, $\sum_iw_i=1$, and
aggregate wealth $W_M^0=\sum_aW_a^0$ measured in the same units as $W_a^0$, gives
%
\begin{equation}
...
=\vec t^{\top}\vec t-\vec t^{\top}A\vec t$, which is \eqref{eq:dirichlet}. The
last claim follows from the example of Proposition~\ref{prop:reversal}, whose
column sums are $(2,1,0)$.
\end{proof}
...
investor receiving as much of it as he gives. When attention is unbalanced the
identity fails and the sign is free, which is the content of the counterexample
below.
...
$\{(\theta+\lambda)^{-1}\}$ uniformly integrable. Suppose the networks satisfy
%
\begin{equation}
...
\begin{proof}
By Lemma~\ref{lem:bound}, $\|\vec c\|_\infty\leq1/\underline\theta$ uniformly in
$m$ and $\lambda$. Hence
...
Under the dense i.i.d.-weight design specified below it becomes rare as the
number of investors grows: the rows of $A$ approach the uniform distribution, so
by Remark~\ref{rem:consistency} the design converges to the mean-field case, in
which \eqref{eq:dirichlet} applies and the effect is non-negative. Nothing in
this licenses a claim about networks in general. Real influence structures carry
...
per cent at $m=6$ and indistinguishable from zero at $m\geq10$. The decay in $m$
is specific to this ensemble; see Remark~\ref{rem:blocks}. Reversal is also
rarer at $\sigma=0.8$, but the two rows are not a clean comparative static in
dispersion: the truncation at $0.15$ binds for $14.4$ per cent of draws at
$\sigma=0.8$ against $1.7$ at $\sigma=0.4$, which moves the shape of the
distribution and raises $\E[\theta]$ from $1.00$ to $1.06$. We report both rows
and draw no conclusion from their difference.}
\label{tab:reversal}
\end{table}
...
network in a way that two limits describe completely. The marginal effect at
...
weighted by influence rather than by population. Between them these two
statements account for the mean-field interpolation from the harmonic to the
arithmetic mean, for non-monotonicity and sign reversal, and for the complete
inertness of the aggregate under perfect homophily by type. What decides which
case obtains is the correlation between net risk aversion and stationary
influence.

Two boundaries of the result should be kept in view. It concerns the effective
market coefficient, not asset prices: the model delivers a mean--covariance
relation conditional on the covariance matrix, and closing it into a full
one-period economy with exogenous quantities and endogenous prices is the
obvious next step. And $\theta_a$ is risk aversion net of the comparison, not
the coefficient of absolute risk aversion in own wealth, so ``common risk
aversion'' in Theorem~\ref{thm:neutral} means a common value of the product
$\alpha_a(1-\gamma_a)$, along which stronger comparison motives come paired with
greater own-wealth risk aversion.
...
Lemma~\ref{lem:bound} and each of the three cases of \eqref{eq:intense}. The scripts depend only on
