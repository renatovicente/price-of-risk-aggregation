...
a network of peers. Equilibrium demand scales solve a Katz--Bonacich system,
and prices are explicit in one scalar, the effective market coefficient, which
is itself independent of prices; the market Sharpe ratio is proportional to it.
We characterise how the network aggregates risk aversion through two limits
available for every network. At zero comparison intensity the marginal effect is
a quadratic form in risk tolerances, which is a Dirichlet form --- hence
non-negative --- exactly when attention is balanced, in the sense that the
influence matrix is doubly stochastic. Under intense comparison the coefficient
converges to a stationary average of individual coefficients, weighted by
influence rather than by population. The two limits organise the rest.
Comparison lowers the effective coefficient, and so raises prices, whenever
stationary influence favours the less risk averse; it interpolates from the
harmonic to the arithmetic mean when influence is spread evenly; and it does
nothing at all when the network splits into blocks homogeneous in type. A
...
\end{equation}
%
which holds for every $\theta_M$ small enough and is the only parameter
restriction the closure imposes. All the results below concern $\theta_M$, so
...
comparative static in the price of risk, in the strict sense, and by
\eqref{eq:price} a fall in $\bth$ raises every price whose row of $V\vec\nu$ is
positive.
...
aggregate initial wealth, and comparison lowers the riskless rate whenever it
lowers $\bth$.
\end{remark}
...
no wealth effects and $\theta_M$ therefore does not depend on prices. A network
that lowers $\bth$ raises every asset price and compresses every risk premium.
